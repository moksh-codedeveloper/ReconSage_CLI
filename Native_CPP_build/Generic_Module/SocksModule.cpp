 ...
 */
#include "SocksModule.hpp"

string SocksProxy::msg_according_to_code(uint8_t code)
{
    if (code == 0x01)
        return error_msg[0];
    if (code == 0x02)
...
SocksProxy::SocksProxy(char _domain[256], char _proxy_host[256], int _timeout, int _proxy_port, int _proto_port)
{
